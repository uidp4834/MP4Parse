...
{
   printf( "detected moov hidden in a free box ...,%s\n",__func__ );
   /* XXX sometime moov is hiden in a free box */
   if( p_box->p_father &&
      p_box->p_father->i_type == ATOM_root &&
      p_box->i_type == ATOM_free )
...
unknown:
   if MP4_BOX_TYPE_ASCII()
      printf(
      "unknown box type %4.4s , %d,(incompletely loaded2)\n",
      (char*)&p_box->i_type,__func__);
   else
	   printf(
      "unknown box type c%3.3s (incompletely loaded3)",
      (char*)&p_box->i_type+1 );
...
mp4_box_t * MP4_BoxSearchBox(mp4_box_t *p_head, uint32_t i_type)
{
    ...
    while (cur != NULL)
    {
        ...
        {
      			printf("current box is %c%c%c%c,%d\n",cur->i_type&0x000000ff,(cur->i_type&0x0000ff00)>>8,(cur->i_type&0x00ff0000)>>16,(cur->i_type&0xff000000)>>24,__LINE__);
        ...
        else
        {
                ...
                if(cur->p_next !=NULL){
                  ...
                  else if(cur->p_father != NULL){
                    printf("lyk-go=%d\n",__LINE__);
                    if(cur->p_father->p_next)
                        cur = cur->p_father->p_next;
                    else if(cur->p_father->p_father != NULL){
